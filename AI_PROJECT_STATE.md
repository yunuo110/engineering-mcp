...
- Persistence: `node:sqlite` (`DatabaseSync`) with WAL, foreign keys, 5s busy timeout, and `user_version = 3`.
...
- Retry/requeue remains explicit through owner `resume_task`; V1.5.2 does not automatically retry.
...
- If a worker crashes, its task remains RUNNING until an OWNER explicitly recovers it. This is intentional fail-closed behavior; V1.5.2 does not infer worker death from startup.
- V1/V1.5.1 databases are migrated to schema version 3 by adding `execution_instance_id`, `ledger_metadata`, and execution-fencing triggers without destructive resets.
...
None that block V1.5.2 use. Automatic dead-worker detection requires future heartbeat/lease/fencing support and remains out of scope.
